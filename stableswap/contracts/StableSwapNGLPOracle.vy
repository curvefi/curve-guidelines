...
def _portfolio_value() -> uint256:
    return lp_oracle_bisection._portfolio_value(self._scaled_A_raw(), staticcall POOL.price_oracle(0))
...
def lp_price(i: uint256=0) -> uint256:
    ...
    # price_oracle(0) is coin0 per coin1, so convert by multiplying with reciprocal.
    return lp_price_in_coin0 * PRECISION // staticcall POOL.price_oracle(0)
